...
for i = 1:size(all_data,1)
    ...
    if ~isempty(pfile)
        ...
        game_start = pfile.processed_data.interview{1,1};
        game_end_tmp = tail(pfile.processed_data.events.game, 1);
        game_end = game_end_tmp{1,1};
        
        duration = game_end - game_start;
        if duration < 0 
            game_end_tmp = tail(pfile.processed_data.interview, 1);
            game_end = game_end_tmp{1,1};
            duration = game_end - game_start;
        end
        ...
        current_summary.interview_num = size(pfile.processed_data.interview, 1);
        current_summary.word_point_num = size(pfile.processed_data.calibrated_words, 1);
