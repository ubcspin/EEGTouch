...
%%take user input on which din
    while which_din < 1 || which_din > length(dins)
        ...
        if (isempty(which_din) || which_din < 1 || which_din > length(dins) || which_din ~= floor(which_din) )&& ~strcmp(questdlg(['No valid DIN index entered. Please double check value and enter an integer between 1 and ' num2str(length(dins)) '. Do you want to try again?'],'Yes','No','Yes'),'Yes')
...
processed_data.scalars.din_time_ms = dins(which_din);
processed_data.scalars.eeg_start_time_ms = microsecs + secs*1000000 + mins*1000000*60;