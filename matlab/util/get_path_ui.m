% Looks for a file or folder matching certain parameters.
% If the file or folder is not found, prompts user to find it.
% If user refuses to find it, throws exception.

function the_fullpath = get_path_ui(the_trial_directory, the_pattern, the_descript, help_message, isfile, canfail)
            ...
            if ~tf
                if strcmp(questdlg(['No ' the_descript ' selected. Do you want to try again?'],['No ' the_descript ' selected'],'Yes','No','Yes'),'No')
                    if ~canfail  
                        ...
                        indx = 1;
                        the_fullpath(indx).name = "";
                        the_fullpath(indx).folder = "";
                    end    
                end
            end
        end
...
the_fullpath = fullfile(the_path,string(the_name));
end