function status = write_csv(local_paths, trial_data)
...
for i = 1:length(trial_data.events.game_controlled_sound.timestamp_ms)
    big_table.events_game_controlled_sound(trial_data.events.game_controlled_sound.timestamp_ms(i)) = trial_data.events.game_controlled_sound.label(i);
end
...
writetable(big_table,fullfile(csv_dir,['eeg' trial_data.scalars.trial_number '.csv']));
end